...
class MovementOfficerController extends Controller
{
    public function index()
    {
        // جلب الرحلات التي حالتها Available أو In_Progress مرتبة حسب الحالة ووقت البدء
        $trips = Trip::leftJoin('users', 'users.id', 'trips.driver_id')
            ->whereIn('status', ['Available', 'In_Progress'])
            ->orderBy('status', 'desc')
            ->orderBy('stage_1_time', 'desc')
            ->select('trips.*', 'users.name as driver_name')
            ->get();

        return view('movement_officer.home', compact('trips'));
    }
    ...
    public function completedTrips(Request $request)
    {
        ...
        return view('movement_officer.completed-trips', compact(
            'validMonth',
            'months',
            'trips',
            'month',
